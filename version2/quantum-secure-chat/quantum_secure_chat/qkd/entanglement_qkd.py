...
class EntanglementQKD:
    ...
    def generate_key_bytes(self):
        """Generate E91 key and return as bytes"""
        key_bits = self.generate_key()
        return self.bits_to_bytes(key_bits)
    ...
    def estimate_entanglement_quality(self):
        """Estimate entanglement quality based on correlation"""
        if not self.alice_bits or not self.bob_bits:
            return 0.0
        
        matching_bases_indices = [i for i in range(len(self.alice_bases)) 
                                if self.alice_bases[i] == self.bob_bases[i]]
        
        if not matching_bases_indices:
            return 0.0
        
        correlations = sum(1 for i in matching_bases_indices 
                          if self.alice_bits[i] == self.bob_bits[i])
        
        return correlations / len(matching_bases_indices)
